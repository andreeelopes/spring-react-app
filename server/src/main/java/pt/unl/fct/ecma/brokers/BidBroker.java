...
public class BidBroker {
    ...
    public void updateBid(Bid bid) {
        proposalService.getProposal(bid.getProposal().getId());

        if (bidService.bidBelongsToProposal(bid.getProposal(), bid.getBidder()))
            bidService.updateBid(bid);
    }

    public void addBidToProposal(Bid bid) {
        Proposal proposal = proposalService.getProposal(bid.getProposal().getId());
        Employee bidder = employeeService.getEmployee(bid.getBidder().getId());

        if(!bidService.bidBelongsToProposal(proposal, bidder))
